...
def generate_plotly_chart():
    ...
    plates_number = [
        st.session_state["plates_number"][server] for server in server_names
    ]
...
def main():
    ...
    cargo_num = st.sidebar.number_input(
        "Enter Cargo Number:", min_value=0, value=1, step=1
    )
    ...
    with col1:
        for server_name in BASE_URLS.keys():
            ...
            st.markdown(
                f'<div class="server-header" style="color: {server_color}; font-weight: bold;">{server_name}</div>',
            ...
            with button_col1:
                move_down_disabled = (
                    st.session_state["plates_number"][server_name] >= 12
                )
                if st.button(
                    "Move Down Cargo",
                    key=f"move_down_{server_name}",
                    ...
                    if not move_down_disabled:
                        ...
                        if result:
                            ...
                            st.write(
                                f"Response from {server_name} for Moving Down Cargo:"
                            )
                            st.json(result)
                        ...
                        st.warning(
                            f"Maximum number of plates (12) reached for {server_name}!"
                        )
                ...
                if st.button(
                    ...
                    if not move_up_disabled:
                        ...
                        if result:
                            st.session_state["plates_number"][server_name] -= 1
                            st.write(
                                f"Response from {server_name} for Moving Up Cargo:"
                            )
                            st.json(result)
                            fig = generate_plotly_chart()
                            st.plotly_chart(fig, use_container_width=True)
                        ...
                        st.warning(
                            f"Minimum number of plates (0) reached for {server_name}!"
                        )
